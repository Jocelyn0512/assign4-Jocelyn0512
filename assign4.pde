...
void draw() {
   ...
   for (int i = 0; i < soilHealth.length; i++) 
    ...
    for(int i = 0; i < 24; i+=4)
    {
      ...
      {
        if(i<=j){
        image(cabbage,i*SOIL_SIZE, j*SOIL_SIZE);
        }
      }
    }
    ...
    {
     if((playerX+SOIL_SIZE)>(cabbageX)&&(playerX)<(cabbageX+SOIL_SIZE)
     &&(playerY)<(cabbageY+SOIL_SIZE)&&(playerY+SOIL_SIZE)>(cabbageY))
     {
       cabbageX=0;
       cabbageY=0;
       playerHealth++; 
